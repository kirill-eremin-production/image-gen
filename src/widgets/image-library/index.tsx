...
export function ImageLibrary({
  ...
  async function handleDeleteAll(type: "references" | "generated" | "videos", label: string) {
    ...
    <div className="min-h-full bg-white p-4 dark:bg-zinc-900">
      ...
      <div className="flex items-center justify-between pb-2 mb-4 mt-8 border-b-2 border-violet-500">
        ...
        {sortedVideos.length > 0 && (
          <button
            onClick={() => handleDeleteAll("videos", "Все сгенерированные видео")}
            className="flex items-center gap-1 px-2 py-1 text-xs text-zinc-500 hover:text-red-500 transition-colors"
          >
            <Trash2 size={12} />
            Удалить все
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-3">
          {sortedVideos.map((file) => (
            <ImageCard
              ...
              file={file}
              type="videos"
              onReveal={() => handleReveal("videos", file.name)}
              onDelete={() => handleDelete("videos", file.name)}
            />
          ))}
        </div>
      )}
